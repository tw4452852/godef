...
func main() {
	...
	*tflag = *tflag || *aflag || *Aflag
	...
	f, err := parser.ParseFile(types.FileSet, filename, src, parser.ParseComments, pkgScope, types.DefaultImportPathToName)
	if f == nil {
...
func parseLocalPackage(filename string, src *ast.File, pkgScope *ast.Scope, pathToName parser.ImportPathToName) (*ast.Package, error) {
	...
	for _, pf := range list {
		...
		src, err := parser.ParseFile(types.FileSet, file, nil, parser.ParseComments, pkg.Scope, types.DefaultImportPathToName)
		if err == nil {
			pkg.Files[file] = src
		}
	}
